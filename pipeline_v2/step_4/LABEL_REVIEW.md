...
### Iter 14 — geometry diagnosis + snap fix; Volume_1 to gold; continuation regressed (2026-06-07, 90-min session)
**Corpus ~400 gold** (A1 52 / A2 76 / A3 46 / V1 273; V1 hand-reviewed by David). V1 promoted to
the few-shot pool (now 396 typed examples; V1 adds 115 letter / 87 notebook / 67 other / 5 card).
`Volume_1/page_148` excluded everywhere (editor anomaly — David: skip). A3 PDF vintage repaired on
this machine (copied committed auto_labeled PDFs over the stale crops → B7 green).

**OBJECTIVE GEOMETRY DIAGNOSIS** (`experiments/geom_diagnose.py`, ink vs gold, 2774 matched boxes —
NOT semantic judgment, which David has rightly called unreliable). Corrects the working premise:
- **Right-edge clipping (David's #1 worry): RARE — 0.8%** of boxes miss gold ink on the right.
- **Vertical OVER-coverage is 8× more common — top 6.1% / bottom 6.6%**: auto boxes drawn TALLER
  than gold, grabbing neighbour lines. This is the real bulk of "boxes cover too much / pick up
  other boxes' text."

**SNAP FIX (shipped to production, validated).** Root cause of the clipping that does happen: snap
computed horizontal extent from a FULL-HEIGHT COLUMN PROJECTION, which washes out a single long
line (only 1 row tall) so the right edge never reaches it. Research (deep-research, adversarially
verified: RLSA / Docstrum / per-line extent) + a gold-grounded recovery harness
(`experiments/snap_lab.py`, perturb gold → snap → measure, 976 boxes) →
`snap_polygon_to_ink` now fits the horizontal extent PER TEXT ROW (longest line drives the edge)
with a wider horizontal search reach (margin_frac 0.22 → 0.45 horizontal) and the vertical margin
UNCHANGED. Harness: clip residual 13.0→8.4px (−35%), over-coverage 28.7→27.5, under-coverage
6.4→8.0 (negligible — no new text loss; reducing the vertical margin was tried and REJECTED: it
clipped text vertically on 85% of boxes). Real auto_labeled re-snap on clip-prone pages: recovers
10/22 right-clips (−45%). **Over-coverage is deliberately left to the new editor arrow-key shrink**
(Iter: editor Arrow=shrink / Ctrl+Arrow=expand / Shift+Arrow=move) — safe, since over-coverage is
one keystroke to fix whereas auto-tightening risks clipping text.

**FEW-SHOT QUESTION ANSWERED (research, don't spend API):** many-shot in-context learning improves
multimodal CLASSIFICATION (log-linear to ~2000 shots) but the same studies explicitly find
bounding-box/LOCALIZATION did NOT gain. So expanding demos (12→20→32) is unproven for box geometry;
the right lever is deterministic post-processing (the snap fix), not more demos. (Pool grew anyway
via V1 promotion; per-type routing means notebook targets now draw from 87 notebook demos.)

**CONTINUATION TUNE r2 — REGRESSED (honest negative result).** Per plan, continued cont-r1 +2 epochs
on the full 408-page set (cold-20 A2 held out, page_148 excluded). Cold-20 A2: ckpt1(ep1) PQ 0.630 /
strict 0.543 > ckpt3(ep2) 0.578 / 0.461 (overfit after ep1, as cont-r1 did) — but BOTH below cont-r1's
0.807 / 0.692. The cheap +2-epoch continuation HURT, likely (a) too-aggressive a step on a 4×-larger
set and (b) distribution shift toward the now-dominant 271 V1 notebook pages (cold test is A2; V1
performance unmeasured — a V1 cold slice would disambiguate). **cont-r1 stays the best tuned model.**

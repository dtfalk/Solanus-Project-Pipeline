        ...
        try:
            r = c.models.generate_content(model=model, contents=[
                ...
                config=types.GenerateContentConfig(system_instruction=sysp, temperature=0.0,
                    ...
                    response_schema=AL.DocumentsResponse))   # force OUR schema, like production
            txt=(r.text or "").strip()
            try:
...
def cmd_teardown(args):
    """Delete the tuned model + serving endpoint so NOTHING is left deployed (the
    directive). Correct order (validated): undeploy model from endpoint -> delete
    endpoint -> delete model. The genai SDK doesn't expose endpoint ops, so use
    gcloud ai (already authed via ADC). Gemini tuned endpoints are serverless, but
    we delete them regardless to be clean."""
    ...
    if ep_id:
        ...
        if dmid:
            _sh(["gcloud","ai","endpoints","undeploy-model",ep_id,
                 f"--deployed-model-id={dmid}",*base]); print("undeployed", dmid)

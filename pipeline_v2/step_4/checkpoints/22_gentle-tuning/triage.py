...
def main():
    ...
    # TUNED-shadow routes (GENTLE plan Phase 4, gate PASSED 2026-06-06: tuned shadow
    # Spearman vs David's actual edits 0.739 > flash-lite shadow 0.644 on cold A2).
    # Tuned endpoints can't persist (they bill hourly), so the tuned shadow is used
    # via (a) precomputed predictions, or (b) a live endpoint while one is up:
    ap.add_argument("--shadow-pred", default=None,
                    help="dir of precomputed shadow predictions (<dir>/<vol>/page_X/page_X.json) "
                         "— e.g. a tuned model's eval output; skips shadow inference")
    ap.add_argument("--shadow-tuned-endpoint", default=None,
                    help="live tuned-model endpoint to use as the shadow (bare prompt)")
    ap.add_argument("--out-name", default="triage.txt",
                    help="worklist filename under qa_output/<vol>/ (default triage.txt)")
    ...
    if args.shadow_pred:
        shadow_dir = Path(args.shadow_pred)
        if not shadow_dir.is_absolute():
            shadow_dir = SCRIPT_DIR / shadow_dir
        shadow_dir = shadow_dir / args.volume
    elif args.shadow_tuned_endpoint:
        # bare-prompt tuned shadow via the sweep helper (no few-shot, [0,1000]->px)
        spec = importlib.util.spec_from_file_location("sweep", SCRIPT_DIR / "experiments" / "sweep.py")
        sweep = importlib.util.module_from_spec(spec); spec.loader.exec_module(sweep)
        sysp = sweep._etd()._tuning_system_prompt()
        c = sweep._client()
        shadow_dir = SHADOW_ROOT / f"{args.volume}__tuned"
        for pn in sorted(page_pdf):
            name = f"page_{pn:03d}"
            if (shadow_dir / name / f"{name}.json").exists():
                continue
            print(f"[tuned shadow] {args.volume}/{name}", flush=True)
            try:
                sweep._label_one(c, args.shadow_tuned_endpoint, sysp, args.volume, name, shadow_dir)
            except Exception as exc:
                print(f"  tuned shadow failed {name}: {exc}")
    else:
        ab = _load_ab()
        ...
        shadow_dir = SHADOW_ROOT / args.volume
    ...
    for pn in sorted(page_pdf):
        ...
        sj = shadow_dir / name / f"{name}.json"
    ...
    print(f"\ntriage written: {out_dir/args.out_name}")

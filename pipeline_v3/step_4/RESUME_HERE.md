...
## 2026-06-07 — geometry fix + V1 gold + continuation regressed (LABEL_REVIEW Iter 14)
Corpus **~400 gold** (V1's 273 hand-reviewed pages promoted to the pool → 396 typed examples;
`Volume_1/page_148` excluded everywhere — editor anomaly David flagged). A3 PDF vintage repaired
locally (B7 green).
- **SNAP FIX SHIPPED (production):** objective ink diagnosis (`experiments/geom_diagnose.py`, 2774
  boxes) found over-coverage (top/bottom ~6.5%) is 8× more common than right-clip (0.8%). Fixed the
  clip cause (full-height column projection washing out long lines) → `snap_polygon_to_ink` now fits
  horizontal extent PER TEXT ROW + wider horizontal reach, vertical margin unchanged. Validated
  (harness `experiments/snap_lab.py`: clip −35%; real re-snap: −45%; zero new text loss). Over-
  coverage is left to the editor arrow keys (Arrow=shrink / Ctrl=expand / Shift=move, shipped).
- **Few-shot scaling question: ANSWERED by research** — more demos help classification, NOT bbox
  localization. Don't expand demo count to fix geometry; the snap fix is the lever.
- **Continuation tune r2 REGRESSED:** +2ep on 408pg → cold-20 A2 PQ 0.630 (ckpt1) < cont-r1 0.807.
  Distribution shift toward V1 + too-big a step. **cont-r1 stays best.** 0 endpoints / parked:
  cont-r1, gentle-G1. Next tune: fresh gentle tune on full 400 w/ stratified multi-volume holdout,
  or +1ep val-selected continuation.

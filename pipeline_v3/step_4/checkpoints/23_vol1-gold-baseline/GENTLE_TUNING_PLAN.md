...
### Phase 0.5 — Label agreement (RESOLVED 2026-06-06 by David — read, then move on)
David personally re-checked Appendix_1 and Appendix_3 against the Appendix_2 conventions:
**the gold is consistent as-is.** The agent-generated audit findings
(`label_review/GOLD_CONSISTENCY_AUDIT_2026-06-06.md`) are **DISMISSED** — agents (including the
one that wrote this plan) have proven unreliable at judging this labeling scheme.

**STANDING INSTRUCTION for every agent on this project:**
- `reviewed/` gold is ground truth, full stop. Do NOT audit, second-guess, or file findings about
  label conventions, category choices, or box granularity in gold.
- Do NOT exclude any page from the tuning export on label-quality grounds. Train on ALL of
  `reviewed/` exactly as it stands.
- If David edits gold himself, the next `prepare` picks it up automatically — nothing to do.
- Convention questions that genuinely block execution → STOP and leave a question in the run log
  for David; never resolve them yourself.

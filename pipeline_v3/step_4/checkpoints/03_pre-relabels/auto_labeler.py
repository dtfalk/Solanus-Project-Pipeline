...
def _merge_added_boxes(documents: dict, new_docs: dict) -> int:
    """Add boxes from new_docs into the existing documents, assigning each to the
    document whose vertical span contains it (nearest otherwise). Never creates a
    new document, so num_documents is unchanged. Returns the number of boxes added."""
    if not documents:
        return 0
    spans = {}
    for dname, doc in documents.items():
        ys = [v for _cat, polys in doc.items() for b in polys
              for v in (_bbox_of(b["vertices"])[1], _bbox_of(b["vertices"])[3])]
        if ys:
            ...
            for b in polys:
                ...
                for dname, (y0, y1) in spans.items():
                    ...
                    if best is None or dist < best:
                        best, tgt = dist, dname
                documents[tgt].setdefault(cat, []).append(b)
                added += 1
    return added
...
def coverage_backstop(
    ...
    labels and num_documents are never touched). Returns (n_added, in_tok, out_tok)."""
    ...
    added = _merge_added_boxes(documents, new_docs)
    return added, in_tok, out_tok
...
def process_page(
    ...
    if snap:
        ...
        if backstop:
            try:
                n_bs, bs_in, bs_out = coverage_backstop(
                    documents, full_gray, target_img, client, model_name, source_w, source_h)
                input_tokens  += bs_in
                output_tokens += bs_out
                if n_bs:
                    log.info("  coverage backstop: added %d box(es) for uncovered ink", n_bs)
            except Exception as exc:
